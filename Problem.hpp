...
struct QProblem {
    ...
    double f(const arma::vec &x) {
        return arma::dot(x, Q * x) + arma::dot(u, x);
    }
...
struct QResult {
    arma::vec x; // Solution.
    unsigned int nIter; // Total number of iterations of the PGM.
    std::vector<double> history; // History of the objective function values.
};
...
QResult PGM(QProblem p, unsigned int maxIter, double alphaInit, double tau,
